      ...
      <div className="px-6 space-y-6">
            ...
            {upcomingReminders.map(task => (
              <div key={task.id} className="glass-panel rounded-3xl p-5 flex items-center justify-between shadow-sm hover:scale-[1.01] transition-transform cursor-pointer">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-sm font-black text-[var(--text-main)] leading-tight">{task.title}</h4>
                ...
                <button onClick={() => onToggleTask(task.id)} className="w-10 h-10 bg-rose-500/20 hover:bg-rose-500/30 rounded-xl text-rose-500 transition-all active:scale-90 flex items-center justify-center">
            ...
            <div className="glass-panel rounded-[2.5rem] p-6 shadow-sm flex flex-col hover:shadow-lg transition-all h-full">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">Приоритеты</h3>
                </div>
                <button onClick={() => onNavigate('planner')} className="text-[9px] font-black text-[var(--accent)] uppercase tracking-widest">План</button>
              </div>

              <div className="space-y-4">
                {todayTasks.length > 0 ? todayTasks.map(task => (
                  <div key={task.id} className="flex items-center gap-4 group cursor-pointer" onClick={() => onToggleTask(task.id)}>
                    <div className="w-6 h-6 rounded-lg border border-[var(--border-color)] flex items-center justify-center shrink-0 group-hover:border-[var(--accent)] transition-colors">
          ...
          <section>
            <div className="glass-panel rounded-[2.5rem] p-6 shadow-sm flex flex-col h-full relative overflow-hidden group hover:shadow-lg transition-all bg-gradient-to-br from-[var(--accent)]/5 to-transparent">
              <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-[9px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em]">Поток Мыслей</h3>
              </div>

              {recentThoughts.length > 0 ? (
                <div className="flex flex-col flex-1">
                  <p className="text-sm text-[var(--text-main)] font-serif italic line-clamp-3 opacity-90 leading-relaxed mb-4">
                    "{recentThoughts[0].content}"
                  </p>
                  <button onClick={() => onNavigate('chat')} className="mt-auto py-3 bg-[var(--accent)] text-white rounded-xl text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all shadow-md">
                    Обсудить с Серафимом
                  </button>
                </div>
              ) : (
        ...
        <section>
          <div className="flex items-center justify-between mb-4 px-1">
            ...
            <button onClick={() => onNavigate('projects')} className="text-[9px] font-black text-[var(--accent)] uppercase tracking-widest">Все</button>
          </div>
          <div className="flex gap-4 overflow-x-auto no-scrollbar pb-4 snap-x">
            {activeProjects.length > 0 ? activeProjects.map(project => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-center mt-4">
                    ...
                    <div className="flex-1 h-1 bg-[var(--bg-main)] rounded-full mx-2 overflow-hidden border border-[var(--border-color)]">
              ...
              <div className="w-full py-12 border border-dashed border-[var(--border-color)] rounded-3xl flex flex-col items-center justify-center opacity-30">
