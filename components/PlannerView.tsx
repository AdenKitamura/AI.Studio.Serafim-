          ...
          <div className="space-y-2">
            {tasksForSelected.length === 0 ? (
              ...
              tasksForSelected.map((task) => (
                <div key={task.id} onClick={() => onToggleTask(task.id)} className={`glass-panel flex items-center gap-4 p-5 rounded-3xl active:scale-[0.99] transition-all cursor-pointer hover:border-[var(--accent)]/30 ${task.isCompleted ? 'opacity-40 grayscale' : ''}`}>
