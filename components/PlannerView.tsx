      ...
      <div className="px-6 py-4 sticky top-0 z-30 bg-[var(--bg-main)]/95 backdrop-blur-xl border-b border-[var(--border-color)] transition-all duration-300 mt-2">
        <div className="cursor-pointer group" onClick={() => setShowCalendar(!showCalendar)}>
            <div className="flex items-center gap-3">
              ...
              <div className={`p-1.5 rounded-full bg-[var(--bg-item)] border border-[var(--border-color)] transition-transform duration-300 ${showCalendar ? 'rotate-180 bg-[var(--accent)] border-[var(--accent)] text-[var(--text-on-accent)]' : 'text-[var(--text-muted)]'}`}>
                  <ChevronDown size={16} />
              </div>
            </div>
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 animate-in zoom-in-95 duration-200">
          <div className="w-full max-w-sm glass-card rounded-[3rem] p-8 shadow-2xl border border-white/10">
            ...
            <button onClick={handleAddTask} disabled={!newTaskTitle.trim()} className="w-full py-5 bg-[var(--accent)] text-[var(--text-on-accent)] rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-[var(--accent-glow)] active:scale-95 transition-all disabled:opacity-20">Зафиксировать</button>
          </div>
        </div>
      )}
      ...
      {editingTask && (
        <div className="fixed inset-0 z-[150] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 animate-in zoom-in-95 duration-200">
          <div className="w-full max-w-sm glass-card rounded-[3rem] p-8 shadow-2xl border border-white/10">
            ...
            <div className="flex gap-3 mt-10"><button onClick={handleDeleteTask} className="px-5 py-4 bg-red-500/10 text-red-500 rounded-2xl flex items-center justify-center hover:bg-red-500 hover:text-[var(--text-on-accent)] transition-all"><Trash2 size={20} /></button><button onClick={handleSaveEdit} className="flex-1 py-4 bg-[var(--accent)] text-[var(--text-on-accent)] rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-[var(--accent-glow)] active:scale-95 transition-all flex items-center justify-center gap-2"><Save size={18} /> Сохранить</button></div>
          </div>
        </div>
      )}
    </div>
