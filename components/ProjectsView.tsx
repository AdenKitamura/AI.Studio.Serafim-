...
const ProjectsView: React.FC<ExtendedProjectsViewProps> = ({ 
  ...
  const saveTask = (isUpdate: boolean) => {
    ...
    if (isUpdate && viewingTask) {
      ...
      setViewingTask({ ...viewingTask, ...taskData }); // Local update for immediate feedback
    } else {
      onAddTask({
        id: Date.now().toString(),
        ...taskData,
  ...
  if (selectedProject) {
    ...
    // Filter thoughts by ACTIVE BOARD ID
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {activeTab === 'canvas' && (
            <div className="h-full w-full flex flex-col">
              {/* Board Switcher */}
              <div className="absolute top-4 left-4 z-40 flex items-center gap-2 glass-panel p-1 rounded-xl">
                  {selectedProject.boards?.map(board => (
                      <button 
                        key={board.id}
                        onClick={() => setActiveBoardId(board.id)}
                        className={`px-3 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${activeBoardId === board.id ? 'bg-[var(--accent)] text-white' : 'text-[var(--text-muted)] hover:text-white'}`}
                      >
                          {board.title}
                      </button>
                  ))}
                  <button onClick={handleAddBoard} className="p-2 text-[var(--text-muted)] hover:text-white"><Plus size={14}/></button>
              </div>
          ...
          <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in zoom-in-95">
            <div className="w-full max-w-sm glass-card rounded-[2.5rem] p-6 shadow-2xl border border-white/10">
               ...
               <input 
                   ...
                   className="w-full bg-transparent text-xl font-bold text-[var(--text-main)] outline-none pb-2 border-b border-[var(--border-color)] mb-4"
               />
            ...
            <div className="fixed inset-0 z-[70] bg-black/80 backdrop-blur-md flex justify-end animate-in slide-in-from-right duration-300">
                <div className="w-full max-w-md h-full bg-[var(--bg-main)] border-l border-[var(--border-color)] flex flex-col shadow-2xl">
                    ...
                    <div className="p-6 border-b border-[var(--border-color)] flex justify-between items-start">
                        <div className="flex-1 mr-4">
                            <textarea 
                                value={taskForm.title}
                                onChange={e => setTaskForm({...taskForm, title: e.target.value})}
                                className="w-full bg-transparent text-2xl font-black text-[var(--text-main)] outline-none resize-none leading-tight"
                                rows={2}
                            />
                            <div className="flex items-center gap-2 mt-2">
                                <select 
                                    ...
                                    className="bg-[var(--bg-item)] text-[var(--text-muted)] text-[10px] font-bold uppercase py-1 px-2 rounded-lg border border-[var(--border-color)] outline-none"
                                >
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-8 no-scrollbar">
                        ...
                        <div>
                            ...
                            <div className="flex gap-3">
                                <div className="flex items-center gap-2 bg-[var(--bg-item)] p-3 rounded-xl border border-[var(--border-color)] flex-1">
                                    <Calendar size={16} className="text-[var(--accent)]" />
                                    <input type="date" value={taskForm.date} onChange={e => setTaskForm({...taskForm, date: e.target.value})} className="bg-transparent text-sm text-[var(--text-main)] outline-none w-full"/>
                                </div>
                                <div className="flex items-center gap-2 bg-[var(--bg-item)] p-3 rounded-xl border border-[var(--border-color)] flex-1">
                                    <Clock size={16} className="text-[var(--accent)]" />
                                    <input type="time" value={taskForm.time} onChange={e => setTaskForm({...taskForm, time: e.target.value})} className="bg-transparent text-sm text-[var(--text-main)] outline-none w-full"/>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            {taskForm.attachments.length > 0 ? (
                                ...
                                <div onClick={() => detailFileInputRef.current?.click()} className="h-24 border-2 border-dashed border-[var(--border-color)] rounded-2xl flex flex-col items-center justify-center text-[var(--text-muted)] hover:border-[var(--accent)] hover:text-[var(--text-main)] cursor-pointer transition-all">
                                    <Paperclip size={20} className="mb-2" />
                                    <span className="text-[9px] font-bold uppercase">Перетащите файлы</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                placeholder="Детали задачи, подзадачи или мысли..."
                                className="w-full bg-[var(--bg-item)] border border-[var(--border-color)] rounded-2xl p-4 text-sm text-[var(--text-main)] min-h-[150px] outline-none focus:border-[var(--accent)] resize-none"
                            />
                        </div>

                        <button 
                            onClick={() => { onDeleteTask(viewingTask.id); setViewingTask(null); }}
                            className="w-full py-4 border border-red-500/30 text-red-500 rounded-2xl font-bold uppercase text-xs hover:bg-red-500/10 transition-all"
                        >
                            Удалить задачу
                        </button>
                    </div>
                    ...
                    <div className="p-4 border-t border-[var(--border-color)] bg-[var(--bg-main)]">
                        <button 
                            onClick={() => { saveTask(true); setViewingTask(null); }}
  ...
  // ... (Keep simple list render or replace if needed, but detail view is now handled via modal)
  return (
