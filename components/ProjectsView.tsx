  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-main)] p-6 overflow-y-auto no-scrollbar max-w-[100vw] overflow-x-hidden">
        <div className="max-w-4xl mx-auto w-full pt-6">
            <div className="flex items-center justify-between mb-8"><div><h2 className="text-3xl font-black text-[var(--text-main)] tracking-tighter mb-1">Проекты</h2><p className="text-[var(--text-muted)] text-sm font-bold opacity-60">Центр управления задачами</p></div></div>
        ...
        {isCreatingProject && (<div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-md flex items-center justify-center p-6 animate-in zoom-in-95"><div className="w-full max-w-sm glass-card rounded-[2.5rem] p-8 border border-white/10 shadow-2xl"><div className="flex justify-between items-center mb-8"><span className="text-[10px] font-black text-[var(--accent)] uppercase tracking-[0.2em]">Создание проекта</span><button onClick={() => setIsCreatingProject(false)}><X size={24} className="text-[var(--text-muted)] hover:text-white" /></button></div><div className="space-y-6"><input autoFocus value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Название проекта" className="w-full bg-transparent text-2xl font-bold text-white outline-none border-b border-white/10 pb-2 placeholder:text-white/20" /><textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Краткое описание..." className="w-full bg-[var(--bg-item)] rounded-xl p-4 text-sm text-white outline-none border border-white/10 resize-none h-24" /><div><label className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-3 block">Цвет обложки</label><div className="flex flex-wrap gap-2">{COLORS.map(c => (<button key={c} onClick={() => setNewColor(c)} className={`w-8 h-8 rounded-full transition-all ${newColor === c ? 'scale-125 ring-2 ring-white' : 'opacity-40 hover:opacity-100'}`} style={{ backgroundColor: c }} />))}</div></div></div><button onClick={handleCreateProject} disabled={!newTitle.trim()} className="w-full mt-8 py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-50">Создать</button></div></div>)}
    </div>
  );
};
