...
const QuotesLibrary: React.FC<QuotesLibraryProps> = ({ myQuotes, onAddQuote, onDeleteQuote, onClose }) => {
  ...
  const dailyQuotes = useMemo(() => {
      ...
      const shuffled = [...all].sort((a, b) => { const x = Math.sin(seed++) * 10000; const y = Math.sin(seed++) * 10000; return (x - Math.floor(x)) - (y - Math.floor(y)); });
        ...
        <div className="flex-1 overflow-y-auto p-4 no-scrollbar">
            {activeTab === 'daily' && (
                <div className="space-y-6 pb-20 animate-in fade-in">
                    <div className="text-center mb-6"><p className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)] opacity-60">{format(new Date(), 'eeee, d MMMM', { locale: ru })}</p></div>
