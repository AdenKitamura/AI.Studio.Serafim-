...
import { Quote as QuoteIcon, X, Download, Search, Sparkles, BookOpen, Calendar } from 'lucide-react';
...
const QuotesLibrary: React.FC<QuotesLibraryProps> = ({ myQuotes, onAddQuote, onDeleteQuote, onClose }) => {
  ...
  // Generate daily quotes deterministically based on date string
        ...
        <div className="p-4 border-b border-[var(--border-color)] flex justify-between items-center bg-[var(--bg-main)]/90 backdrop-blur-md sticky top-0 z-20 shadow-sm">
            <div className="flex items-center gap-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">Вдохновение на сегодня</p>
                </div>
            </div>
            ...
            {activeTab === 'daily' && (
                <div className="space-y-6 pb-20 animate-in fade-in">
                    ...
                    {dailyQuotes.map((quote) => {
                         ...
                         return (
                            <div key={quote.id} className="glass-panel p-8 rounded-3xl relative border border-pink-500/20 shadow-[0_10px_40px_rgba(236,72,153,0.1)]">
                                <QuoteIcon className="absolute top-6 left-6 text-pink-500/20" size={40} />
                                <p className="text-[var(--text-main)] font-serif italic text-xl leading-relaxed mb-6 relative z-10 text-center">
                                    "{quote.text}"
                                </p>
                                <div className="flex flex-col items-center">
                                    <div className="h-px w-10 bg-pink-500/50 mb-3"></div>
                                    <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">{quote.author}</span>
                                </div>
                                <div className="absolute bottom-4 right-4">
                                     {isSaved ? (
                                        <div className="p-2 bg-green-500/10 rounded-full text-green-500"><Sparkles size={16} /></div>
                                     ) : (
                                        <button onClick={() => handleSave(quote)} className="p-2 bg-[var(--bg-item)] hover:bg-pink-500 hover:text-white rounded-full text-[var(--text-muted)] transition-all shadow-lg border border-[var(--border-color)]">
                                            <Download size={16} />
                                        </button>
                                     )}
                                </div>
                ...
                <div className="space-y-6 animate-in fade-in">
                    <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                         {CATEGORIES.map(cat => (
                             <button
                               ...
                               className={`px-4 py-2 rounded-full text-xs font-bold uppercase whitespace-nowrap border transition-all ${selectedCategory === cat.id ? 'bg-[var(--text-main)] text-[var(--bg-main)] border-[var(--text-main)]' : 'glass-panel text-[var(--text-muted)] border-[var(--border-color)]'}`}
                             >
                                 {cat.label}
                             </button>
                         ))}
                    ...
                    <div className="grid grid-cols-1 gap-4 pb-20">
                        {libraryQuotes.map((quote) => {
                            const isSaved = personalQuotes.some(pq => pq.content === quote.text);
                            return (
                                <div key={quote.id} className="glass-panel p-6 rounded-2xl relative hover:border-[var(--accent)]/30 transition-all">
                                    <p className="text-[var(--text-main)] font-serif italic text-lg leading-relaxed mb-4 opacity-90">
                                        "{quote.text}"
                                    </p>
                                    <div className="flex justify-between items-end border-t border-[var(--border-color)] pt-4 mt-2">
                                        ...
                                        {isSaved ? (
                                            ...
                                            <button 
                                                ...
                                                className="px-3 py-1.5 bg-[var(--bg-item)] hover:bg-pink-600 hover:text-white rounded-lg text-[var(--text-muted)] text-xs font-bold transition-colors flex items-center gap-2 border border-[var(--border-color)] glass-btn"
                                            >
                                                <Download size={14} />
                                                Забрать
                                            </button>
                ...
                <div className="space-y-4 pb-20 animate-in fade-in">
                    {personalQuotes.length === 0 ? (
                        ...
                        personalQuotes.map(q => (
                            <div key={q.id} className="glass-panel p-5 rounded-xl border-l-4 border-l-pink-500 relative">
