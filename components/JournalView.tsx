...
const JournalView: React.FC<JournalViewProps> = ({ journal, tasks = [], onSave }) => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window)) {
      ...
      rec.onend = async () => { setIsRecording(false); setInterimText(''); if (content.length > 20) await handleAutoPolish(); };
  ...
  const handleAutoPolish = async () => {
      ...
      setIsPolishing(true);
      try { const cleaned = await polishTranscript(content); if (cleaned && cleaned !== content) setContent(cleaned); } catch (e) {} finally { setIsPolishing(false); }
  };

  const toggleReflection = () => { setShowReflection(!showReflection); if (!showReflection) setTimeout(() => reflectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100); };
    ...
    <div className="flex flex-col h-full bg-[var(--bg-main)] overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-48 no-scrollbar">
        <div className="max-w-2xl mx-auto py-8 space-y-8">
          ...
          <div className="relative min-h-[40vh]"><div className="w-full bg-transparent text-[var(--text-main)] text-xl font-medium leading-relaxed whitespace-pre-wrap outline-none mb-20">{content}{interimText && <span className="text-[var(--text-muted)] opacity-30 animate-pulse"> {interimText}</span>}{!content && !interimText && <span className="text-[var(--text-muted)] opacity-10 font-serif italic text-2xl">О чем ты думаешь сегодня?..</span>}</div><textarea className="absolute inset-0 w-full h-full opacity-0 cursor-text resize-none" value={content} onChange={e => setContent(e.target.value)} disabled={isPolishing || isRecording} /></div>
          {showReflection && (
              <div ref={reflectionRef} className="animate-in slide-in-from-bottom-5 fade-in space-y-6 pt-8 border-t border-[var(--border-color)] transition-all">
                  <div className="flex items-center gap-2 mb-4"><Sparkles size={18} className="text-[var(--accent)]" /><h3 className="text-xs font-black uppercase text-[var(--accent)] tracking-[0.2em]">Рефлексия дня</h3></div>
