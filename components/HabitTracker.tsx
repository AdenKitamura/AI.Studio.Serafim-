...
const HabitTracker: React.FC<HabitTrackerProps> = ({ habits, selectedDate, onToggle, onAdd, onDelete }) => {
  ...
  return (
    <div className="flex gap-3 overflow-x-auto no-scrollbar py-1">
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-200">
          <div className="w-full max-w-xs glass-card rounded-[2.5rem] p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <span className="text-[10px] font-black text-indigo-400 uppercase tracking-widest">Новая привычка</span>
              <button onClick={() => setIsAdding(false)}><X size={18} className="text-white/20" /></button>
            </div>
            <input 
              autoFocus
              value={newTitle}
              ...
              className="w-full bg-transparent text-lg font-bold text-white outline-none placeholder:text-white/10 mb-6"
            />
            <div className="flex gap-3 mb-8 justify-center">
              {COLORS.map(c => (
                <button 
                  ...
                  className={`w-6 h-6 rounded-full transition-all ${newColor === c ? 'scale-125 ring-2 ring-white' : 'opacity-40'}`} 
            ...
            <button 
              ...
              className="w-full py-4 bg-white text-black rounded-2xl font-black uppercase tracking-widest active:scale-95 transition-all"
            >
              Создать
            </button>
          </div>
