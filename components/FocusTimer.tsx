      ...
      return (
          ...
          `}>
              ...
              <div className="w-full p-8 flex justify-between items-start z-50">
                  <div className="flex gap-4">
                      {timers.map(t => (
                          <button 
                            ...
                            className={`px-4 py-2 rounded-full border transition-all text-sm font-bold uppercase tracking-wider ${activeTimerId === t.id ? 'bg-white text-black border-transparent' : 'text-white/40 border-white/10 hover:text-[var(--text-on-accent)]'}`}
                          >
                              {t.label}
                          </button>
                      ))}
                  ...
                  <div className="flex gap-4">
                      <button onClick={() => setIsLandscape(!isLandscape)} className="p-4 bg-white/5 hover:bg-white/10 rounded-full text-[var(--text-on-accent)] transition-all">
                          <Smartphone size={24} className={isLandscape ? 'rotate-90' : ''} />
                      </button>
                      <button onClick={() => setIsFullscreen(false)} className="p-4 bg-white/5 hover:bg-white/10 rounded-full text-[var(--text-on-accent)] transition-all">
                          <Minimize2 size={24} />
                      </button>
                  </div>
              </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center z-10 w-full px-10">
                  ...
                  <div className="text-[20vmin] leading-none font-bold text-[var(--text-on-accent)] tracking-tighter drop-shadow-2xl whitespace-nowrap tabular-nums">
              ...
              <div className="pb-20 flex gap-8 z-50">
                  <button onClick={() => resetTimer(activeTimer.id)} className="p-6 rounded-full border border-white/20 text-[var(--text-on-accent)]/50 hover:text-[var(--text-on-accent)] hover:border-white transition-all">
                      <RotateCcw size={32} />
                  </button>
                  <button onClick={() => toggleTimer(activeTimer.id)} className={`p-8 rounded-full text-[var(--text-on-accent)] shadow-[0_0_60px_rgba(255,255,255,0.2)] hover:scale-105 active:scale-95 transition-all ${activeTimer.isActive ? 'bg-orange-500' : 'bg-[var(--accent)]'}`}>
              ...
              >
                  ...
                  <div className="flex items-center justify-between">
                      ...
                      <button 
                          ...
                          className={`w-14 h-14 rounded-full flex items-center justify-center text-[var(--text-on-accent)] shadow-lg transition-all active:scale-90 ${timer.isActive ? 'bg-orange-500' : 'bg-[var(--accent)]'}`}
                      >
